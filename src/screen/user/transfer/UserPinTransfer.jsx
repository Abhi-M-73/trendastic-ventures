...
import { findUserByUsernameApi, transferPin } from '../../../api/user.api';

const UserPinTransfer = () => {
    ...
    const handleFindUser = async () => {
        ...
        setIsFetching(true);
        try {
            const res = await findUserByUsernameApi({ userName: username });
            if (res?.data) {
                setUser(res.data);
            } else {
                setUser(null);
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: transferPin,
        onSuccess: (res) => {
            toast.success(res?.message || 'Pin transfer successfully');
            queryClient.invalidateQueries({ queryKey: ['pinTransferHistory'] });
            setPinType("");
            setCount("");
        },
        onError: (error) => {
    ...
    const handleTransferUser = () => {
        ...
        mutate({ receiverId: user?.username, pinType, count: Number(count) });
    };
            ...
            {user && (
                <div className='w-full space-y-4 border border-slate-600 rounded-lg p-5'>
                    ...
                    <ReusableForm
                        ...
                        options={[
                            { label: "Silver Pin", value: "silverPinCount" },
                            { label: "Gold Pin", value: "goldPinCount" },
                            { label: "Diamond Pin", value: "diamondPinCount" },
                        ]}
                    />
...
export default UserPinTransfer;